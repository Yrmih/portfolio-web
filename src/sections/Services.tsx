  ...
  return (
    ...
    >
      ...
      <motion.h2
        ...
        viewport={{ once: true }}
        className="text-4xl font-bold mb-12 text-center text-blue-400 drop-shadow-[0_0_12px_rgba(59,130,246,0.4)] z-10"
      >
        Meus Serviços
      </motion.h2>

      <div className="flex flex-col md:flex-row gap-6 max-w-4xl w-full mb-12 z-10">
        {topServices.map((card) => (
          <ServiceCard key={card.title} {...card} isTopCard />
        ))}
      </div>

      <div className="max-w-6xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 px-6 z-10">
        {services.map((card, i) => (
